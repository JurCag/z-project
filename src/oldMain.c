...
static int set_led_state(const struct shell *sh, size_t argc, char **argv, int state);
...
static int led_period = 1000; // default period
...
static int set_led_state(const struct shell *sh, size_t argc, char **argv, int state)
{
	...
	if (state == 1)
	{
		shell_print(sh, "TURNING LED ON");
	}
	else
	{
		shell_print(sh, "TURNING LED OFF");
	}
...
static int led_on(const struct shell *sh, size_t argc, char **argv)
{
	return set_led_state(sh, argc, argv, 1);
}

static int led_off(const struct shell *sh, size_t argc, char **argv)
{
	return set_led_state(sh, argc, argv, 0);
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(subcmd_led,
							   SHELL_CMD_ARG(on, NULL, "Turn LED on.", &led_on, 1, 0),
							   SHELL_CMD_ARG(off, NULL, "Turn LED off.", &led_off, 1, 0),
							   SHELL_CMD_ARG(toggle, NULL, "Toggle LED with period (ms).", &led_toggle, 1, 1),
							   SHELL_SUBCMD_SET_END /* Array terminated. */
);

SHELL_CMD_REGISTER(led, &subcmd_led, "Control led state", NULL);
